...
describe("setValue", () => {
	...
	it("should set valid uint256 value", () => {
		...
		if (result.success) {
			expect(result.value.get(0)?.value).toBe("1000000");
		}
	});
	...
	it("should preserve existing values when setting new value", () => {
		const currentValues: InputValues = new Map([[0, { value: "1000000" }]]);
...
describe("setValue with dependencies", () => {
	...
	it("should not clear non-dependent values", () => {
		...
		expect(result.success).toBe(true);
		if (result.success) {
			expect(result.value.get(0)?.value).toBe("2000000");
			expect(result.value.get(1)?.value).toBe(
				"0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
			);
			expect(result.value.get(2)?.value).toBe(
